...
namespace VeggieRecipe.WebApp.Pages.AmountIngredients
{
    public class EditModel : PageModel
    {
        ...
        public List<AmountIngredient> AmountIngredientsByRecipe {  get; set; }
        public void OnGet(int id)
        {
            IngredientList = IngredientService.GetAll();
            MesuramentList = MesuramentService.GetAll();
            AmountIngredientsByRecipe = AmountIngredientService.GetAmountIngredientByRecipeId(id);
            foreach (var amount in AmountIngredientsByRecipe)
            {
            Ingredient = IngredientService.GetById(amount.IngredientId.Id);
            Mesurament = MesuramentService.GetById(amount.MesuramentId.Id);
        ...
        public IActionResult OnPost()
        {
            ...
            amountIngredient.Id = Convert.ToInt32(Request.Form["id"]);
            amountIngredient.RecipeId.Id = Convert.ToInt32(Request.Form["recipeId"]);
            amountIngredient.IngredientId.Id = Convert.ToInt32(Request.Form["ingredientId"]);
            amountIngredient.MesuramentId.Id = Convert.ToInt32(Request.Form["mesuramentId"]);
            amountIngredient.Quantity = float.Parse(Request.Form["quantity"]);

            AmountIngredientService.Update(amountIngredient);

            return Redirect("/AmountIngredients/ListAll");
        }
